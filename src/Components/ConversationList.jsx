...
import { getConversationsOf, getUserInfo } from '../Controllers/Controllers'
...
export class ConversationList extends React.Component {
    ...
    componentDidMount() {
        ...
        getConversationsOf(this.props.userInfo.userId)
            .then(async conversations => {
                ...
                return Promise.all(targets).then(tars => {
                    return tars.map((tar, indx) => {
                        console.log(tar)
                        return { userId:tar.userId,avatar: tar.avatar, name: tar.name, conversationId: conversations[indx].conversationId }
                    })
                })
            })
            .then(data => {
