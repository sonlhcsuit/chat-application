...
export class FriendList extends React.Component {
    constructor(props) {
        super(props)
        this.state = { selected: 0,friendsList:[] }
    ...
    render() {
        let fris = this.state.friendsList.map((fr, indx) => {
            return (
                <Friend key={indx} name={fr.name} image={fr.avtUrl} lastmes={fr.lastMessage} selected={indx == this.state.selected} select={() => this.select(indx)} />
            )
        })
        return (
